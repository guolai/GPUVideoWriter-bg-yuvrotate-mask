...
- (CVPixelBufferRef)newRenderdPixelBufferForRequest:(AVAsynchronousVideoCompositionRequest *)request {
    ...
    //当前帧的原始图像
    CVPixelBufferRef resultPixelBuffer = [request sourceFrameByTrackID:trackID];
    if(0) {
        CIImage *image = [CIImage imageWithCVPixelBuffer:resultPixelBuffer];
        CVPixelBufferRef pixelBuffer = [self.renderContext newPixelBuffer];
        [self.ciContext render:image toCVPixelBuffer:pixelBuffer];
        
        CMPersistentTrackID trackID2 = layerInstructions.lastObject.trackID;
        if(trackID2 != trackID) {
            CVPixelBufferRef resultPixelBuffer2 = [request sourceFrameByTrackID:trackID2];
            CIImage *image2 = [CIImage imageWithCVPixelBuffer:resultPixelBuffer2];
            [self.ciContext render:image2 toCVPixelBuffer:pixelBuffer];
        }
